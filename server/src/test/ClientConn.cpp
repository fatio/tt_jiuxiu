...
static ClientConn *g_pConn;

static void clientconn_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
    (void)callback_data;
    (void)handle;
    (void)pParam;
 
    if (g_pConn == NULL) return;
    ...
    case NETLIB_MSG_CLOSE:
        ...
        break;
    default:
        loge("clientconn callback error msg :%d", msg);
        break;
    }
}
...
net_handle_t ClientConn::connect(const string& strIp, uint16_t nPort, uint32_t user_id, uint32_t app_id, uint32_t domain_id, const string& token)
{
	UNUSED(user_id);
	UNUSED(app_id);
   UNUSED(domain_id);
    m_handle = netlib_ssl_connect(strIp.c_str(), nPort, "cacert.pem", clientconn_callback, NULL);
    return  m_handle;
}
...
uint32_t ClientConn::login(uint32_t user_id, uint32_t app_id, uint32_t domain_id, const string& token)
{
    CImPdu cPdu;
