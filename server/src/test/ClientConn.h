...
public:
    ...
    net_handle_t connect(const string& strIp, uint16_t nPort, uint32_t user_id, uint32_t app_id, uint32_t domain_id, const string& token);
    
    virtual void Close();
public:
    uint32_t login(uint32_t user_id, uint32_t app_id, uint32_t domain_id, const string& token);
    ...
    bool isLogined() const { if (m_pCallback != NULL) { return m_pCallback->isLogined(); } else { return false; } }
public:
